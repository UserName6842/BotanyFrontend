  ...
  <div class="wrapper-plant-form">
    ...
    <div class="flex flex-col items-center justify-center">
      <UFormGroup label="Покрытость" name="covered">
        <USelectMenu v-model:model-value="selectValue" :options="optionTypePlant" :searchable="search" class="w-36">
          <template #label>
...
async function search(q: string) {
  ...
  const option = props.optionTypePlant.filter((item) =>
    item.title!.toLowerCase().trim().includes(q.toLowerCase().trim()),
  );
